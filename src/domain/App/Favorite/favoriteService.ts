...
async function loadOffline(): Promise<Favorite[]> {
  try {
...
async function saveOffline(favorites: Favorite[]): Promise<void> {
  try {
...
async function getMyFavorites(type?: FavoriteType): Promise<Favorite[]> {
  try {
...
async function toggleFavorite(data: CreateFavoriteDto): Promise<{
  ...
  try {
    ...
    if (result.action === 'added' && result.favorite) {
      await saveOffline([result.favorite, ...stored]);
    } else if (result.action === 'removed') {
      // Remove do cache local
      const updated = stored.filter(f => !isSameFavorite(f, data));
    ...
    // Toggle offline
    const stored = await loadOffline();
    const existingIndex = stored.findIndex(f => isSameFavorite(f, data));
    ...
    if (existingIndex >= 0) {
      ...
      // Add
      await saveOffline([localFavorite, ...stored]);
...
async function checkFavorite(data: CheckFavoriteDto): Promise<boolean> {
  try {
    ...
    // Verifica offline
    const stored = await loadOffline();
    return stored.some(f => isSameFavorite(f, data));
  }
}
