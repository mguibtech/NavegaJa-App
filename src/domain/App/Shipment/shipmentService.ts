...
async function createShipment(
  ...
  await saveOffline([shipment, ...stored]);

  return shipment;
}

async function createFromFormData(formData: FormData): Promise<Shipment> {
  const shipment = await shipmentAPI.create(formData);
  const stored = await loadOffline();
  await saveOffline([shipment, ...stored]);
  return shipment;
...
async function getTimeline(id: string): Promise<ShipmentTimelineEvent[]> {
  return await shipmentAPI.getTimeline(id);
}

/**
 * Rastrear encomenda
 */
async function trackShipment(trackingCode: string): Promise<TrackShipmentResponse> {
 ...
 * Cancelar encomenda
 */
async function cancelShipment(id: string, reason?: string): Promise<void> {
 ...
 */
async function updateStatus(id: string, status: ShipmentStatus): Promise<Shipment> {
...
async function reportIncident(
  ...
  await shipmentAPI.reportIncident(shipmentId, data);
}

async function validateQR(qrCode: string): Promise<ValidateQRResponse> {
  return await shipmentAPI.validateQR(qrCode);
}
 ...
 */
async function loadOffline(): Promise<Shipment[]> {
  try {
...
async function saveOffline(shipments: Shipment[]): Promise<void> {
  try {
