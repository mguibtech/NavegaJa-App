    ...
    <Box
      ...
      <Box flexDirection="row" alignItems="center" mb="s16">
        <Box
          ...
          height={48}
          borderRadius="s12"
          backgroundColor={config.color as any}
      ...
      {/* Description */}
      <Text preset="paragraphMedium" color={config.color as any} mb="s16">
        {config.description}
      </Text>
      ...
      {safety.warnings.length > 0 && (
        <Box mb="s16">
          <Text preset="paragraphSmall" color={config.color as any} bold mb="s8">
            ⚠️ Avisos:
          </Text>
          {safety.warnings.map((warning, index) => (
            <Box key={index} flexDirection="row" alignItems="flex-start" mb="s6">
              <Text preset="paragraphSmall" color={config.color as any}>
                • {warning}
              </Text>
            </Box>
          ))}
      ...
      <Box
        ...
        paddingTop="s16"
        borderTopWidth={1}
        borderTopColor={config.color as any}
        flexDirection="row"
        alignItems="center"
        justifyContent="center">
        <Icon
          ...
          color={config.color as any}
        />
        <Text preset="paragraphMedium" color={config.color as any} bold ml="s8">
