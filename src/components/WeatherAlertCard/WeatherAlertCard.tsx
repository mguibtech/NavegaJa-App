...
function getSeverityContainerStyle(cfg: typeof SEVERITY_CONFIG[AlertSeverity]) {
  return {
...
function getSeverityBadgeStyle(backgroundColor: string) {
  return {
    backgroundColor,
  };
}

function getSeverityTextStyle(color: string) {
  return {
    color,
  };
}
...
export function WeatherAlertCard({alert, compact = false}: WeatherAlertCardProps) {
  ...
  if (compact) {
    return (
      <Box
        ...
        padding="s12"
        borderRadius="s12"
        style={[styles.compactCard, getSeverityContainerStyle(cfg)]}>
        <Icon name={cfg.icon as any} size={18} color={cfg.color as any} />
        <Box flex={1} ml="s10">
          <Text preset="paragraphCaptionSmall" bold style={getSeverityTextStyle(cfg.color)}>
            {alert.event}
          </Text>
          <Text
            preset="paragraphCaptionSmall"
            style={getSeverityTextStyle(cfg.color)}
            numberOfLines={2}>
            {alert.headline}
          </Text>
        </Box>
  ...
  return (
    <Box
      ...
      borderRadius="s16"
      mb="s12"
      style={[styles.fullCard, getSeverityContainerStyle(cfg)]}>
      ...
      <Box flexDirection="row" alignItems="center" mb="s8">
        ...
        <Box
          ...
          paddingVertical="s4"
          borderRadius="s8"
          style={getSeverityBadgeStyle(cfg.borderColor)}>
          <Text preset="paragraphCaptionSmall" bold style={getSeverityTextStyle(cfg.color)}>
            {cfg.label}
          </Text>
        </Box>
      ...
      {/* Título do alerta */}
      <Text preset="paragraphSmall" bold style={getSeverityTextStyle(cfg.color)} mb="s4">
        {alert.headline}
      </Text>
